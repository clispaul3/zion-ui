...
class ModalContainer extends BaseContainer {
  ...
  createTemplate() {
    ...
    if (width) {
      ...
      this.width = this._calculateSize(0.8)["width"]
    }
    const modalContentStyle = `overflow:hidden;position:absolute;z-index:10000;top:50%;left:50%;background:#fff;transform:translate(-50%,-50%);width:${this.width};height:80%;border:1px solid #ccc;`
    template.innerHTML = `
  ...
  // 放大缩小
  zoomOutIn(direction) {
    let container = document.querySelector("section." + this.iframeId)
    const zoominBnt = document.querySelector("svg.iframe-zoomin-btn")
    const zoomoutBnt = document.querySelector("svg.iframe-zoomout-btn")
  ...
  _calculateSize(scale = 0.8) {
    ...
    if (Math.ceil(width) % 2 == 1) {
      width = Math.ceil(width) + 1
    } else {
      width = Math.ceil(width)
    }

    if (Math.ceil(height) % 2 == 1) {
