  ...
  var _consts = {
      ...
      if (tId.length > 0) {
        ...
        switch (nodeEventType) {
          case "checkNode":
            ...
            break;
          case "mouseoverCheck":
            // zhangyangbin  20200623 不绑定checkbox的mouseover事件
            // nodeEventCallback = _handler.onMouseoverCheck;
            break;
          case "mouseoutCheck":
            // zhangyangbin  20200623 不绑定checkbox的mousecheck事件
    ...
    _view = {
      ...
      makeChkClass: function (setting, node) {
        ...
        // zhangyangbin 20200619 update
        chkName = consts.className.BUTTON + " " + c.DEFAULT + " " + chkName;
        if (setting.check.chkboxType.Y == "" && setting.check.chkboxType.N == "" && setting.check.chkStyle === "checkbox") {
          chkName = chkName.replace("checkbox_true_part_focus", "checkbox_true_full_focus")
          chkName = chkName.replace("checkbox_true_part", "checkbox_true_full")
          chkName = chkName.replace("checkbox_false_part", "")
        }
        return chkName
      },
      ...
      setChkClass: function (setting, obj, node) {
        ...
        const newClassName = view.makeChkClass(setting, node)
      ...
      onMousedownNode: function (eventMouseDown, _node) {
        ...
        function _docMouseUp(event) {
          ...
          if (tmpTarget) {
            ...
            // zhangyangbin 20200620 支持Promise
            const dragResult = tools.apply(setting.callback.beforeDrop, [targetSetting.treeId, nodes, dragTargetNode, moveType, isCopy], true)
            if (dragResult.then) {
              dragResult.then(bool => {
                if (bool == false) {
                  view.selectNodes(sourceSetting, nodes);
                } else {
                  var newNodes = isCopy ? tools.clone(nodes) : nodes;
                  dropCallback(newNodes);
                }
              })
              return
            } else if (dragResult == false) {
              view.selectNodes(sourceSetting, nodes);
              return;
            }
            ...
            function dropCallback(newNodes) {   // zhangyangbin 20200620 添加newNodes参数
              if (isOtherTree) {
                if (!isCopy) {
                  for (var i = 0, l = nodes.length; i < l; i++) {
